...
```bash
SKILL_DIR="$HOME/.codex/skills/x-video-subtitle"
miss=""
...
- `DEEPSEEK_API_KEY` → 写进 `$SKILL_DIR/.env`。
...
**If `READY`**, 跑(`$ARGUMENTS` 可为**本地视频路径**或 **X/Twitter 链接**,可选 `--trim-tail 2` 裁掉结尾秒数):
...
给 X/Twitter 链接时会先用 yt-dlp 下载到 `LIBRARY/<date>_<handle>_<id>/media/video.mp4`;未设置 `LIBRARY` 时默认 `~/x-video-subtitle-library`。受限视频可能需要代理或浏览器登录态。产物落在视频同目录:`<base>.en.srt`(转写留档)、`<base>.zh.ass`(双语字幕,**发布前可手改错词**)、`<base>.zh-hardsub.mp4`(成品)。
